...
def visualize_data_dist_fnln(args, df, K, num_users):
    ...
    try:
        ...
        # 如果 legend 没有 legendHandles 属性，可以尝试其他方式或者跳过
        print("Warning: Could not set legend edge color - legendHandles not found")
...
def prepare_data_digit(args):
    ...
    # train idx（同域不同客户端使用组偏移，必要时环绕补齐）
    num_datasets = len(train_dataset_list)
    idx_batch_train = [[] for _ in range(args.num_clients)]
    user_groups = {}
    for i in range(args.num_clients):
        d = i % num_datasets          # 数据集索引（域）
        g = i // num_datasets         # 客户端组偏移：0组(0-4)，1组(5-9)，2组(10-14)...
        y_train = train_dataset_list[d].labels
        indices = []
        for k in range(args.num_classes):
            idx_k = np.where(y_train == k)[0]
            if len(idx_k) == 0:
                continue
            start = (g * args.n_per_class) % len(idx_k)
            end = min(start + args.n_per_class, len(idx_k))
            slice_k = idx_k[start:end]
            if len(slice_k) < args.n_per_class:
                need = args.n_per_class - len(slice_k)
                wrap = idx_k[:need]
                slice_k = np.concatenate([slice_k, wrap])
            indices.extend(slice_k.tolist())
        idx_batch_train[i] = indices
        user_groups[i] = idx_batch_train[i]

    # test idx（同域不同客户端使用组偏移，必要时环绕补齐）
    idx_batch_test = [[] for _ in range(args.num_clients)]
    user_groups_test = {}
    for i in range(args.num_clients):
        d = i % num_datasets
        g = i // num_datasets
        y_test = test_dataset_list[d].labels
        indices_t = []
        for k in range(args.num_classes):
            idx_k = np.where(y_test == k)[0]
            if len(idx_k) == 0:
                continue
            start = (g * args.n_per_class_test) % len(idx_k)
            end = min(start + args.n_per_class_test, len(idx_k))
            slice_k = idx_k[start:end]
            if len(slice_k) < args.n_per_class_test:
                need = args.n_per_class_test - len(slice_k)
                wrap = idx_k[:need]
                slice_k = np.concatenate([slice_k, wrap])
            indices_t.extend(slice_k.tolist())
        idx_batch_test[i] = indices_t
