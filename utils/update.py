...
class train_update(object):
    ...
    def criterion_alignment(self, features, labels, protos_dict):
        bs = features.shape[0]
        ...
        for i in range(bs):
            label = labels[i].item()
            # 获取该类别的所有原型
            protos = torch.tensor(np.array(protos_dict[label])).to(self.device)  # [num_protos, feature_dim]
            # 计算当前特征与所有原型的余弦相似度
            similarities = F.cosine_similarity(features[i].unsqueeze(0), protos, dim=1)  # [num_protos]
            # 取最大相似度（最近的原型）
            data[i] = similarities.max()
    ...
    def criterion_specific(self, features, labels, protos_dict, N):
        # N = self.args.N
        temperature = self.args.tau
        bs = features.shape[0]
        protos_list = []
    ...
    def update_weights_proposed(self, idx, local_avg_N_protos,local_cluster_protos_N_M, global_N_protos, global_cluster_N_M_protos,global_cluster_N_M_protos_avg, global_avg_cluster_protos, model, global_round, N):
        # Set mode to train model
        model.train()
        epoch_loss = {'total': [], '1': [], '2': []}
        optimizer = torch.optim.SGD(model.parameters(), lr=self.args.lr,  momentum=self.args.momentum, weight_decay=1e-5)
        ...
        for iter in range(self.args.train_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                loss2 = 0 * loss1
                loss3 = 0 * loss1
                if len(local_avg_N_protos) == self.args.num_clients:
                    for i in range(self.args.num_clients):
                        for label in global_N_protos.keys():
                            if label not in local_avg_N_protos[i].keys():
                                local_avg_N_protos[i][label] = global_N_protos[label]
                                local_cluster_protos_N_M[i][label] = [global_cluster_N_M_protos_avg[label]]
                    loss2 += self.criterion_generalized(features, labels, global_N_protos, N)
                    loss2 += self.criterion_specific(features, labels, global_cluster_N_M_protos, N)
                    loss3 += self.criterion_alignment(features, labels, global_avg_cluster_protos)
                    loss2 += loss3
